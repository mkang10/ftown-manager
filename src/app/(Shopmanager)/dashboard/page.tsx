...
const barChartData = {
  ...
  datasets: [{ label: "Revenue", data: [1500, 2500, 1800, 3000, 3200, 2700], backgroundColor: "#4ade80" }],
};
...
const DashboardPageShopManager: React.FC = () => {
  return (
    <DashboardLayout>
      <Box sx={{ p: 3 }}>
        {/* Row 1: Thẻ thống kê */}
          ...
          {stats.map((stat, index) => (
            <Grid item xs={12} sm={6} md={3} key={index}>
              <Card sx={{ display: "flex", alignItems: "center", p: 2 }}>
                <Box sx={{ bgcolor: stat.color + ".light", p: 1.5, borderRadius: "50%" }}>{stat.icon}</Box>
                <CardContent>
...
export default DashboardPageShopManager;
