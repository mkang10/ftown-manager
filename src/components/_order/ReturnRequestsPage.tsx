...
const ReturnRequestsPage: React.FC = () => {
    const [filters, setFilters] = useState<GetReturnRequestsParams>({ pageNumber: 1, pageSize: 10 });
    ...
    const fetchData = useCallback(async () => {
      setLoading(true);
      try {
        const payload: GetReturnRequestsParams = {
          ...filters,
          status: status || undefined,
          returnOption: option || undefined,
    ...
    }, [filters, status, option, fromDate, toDate, orderId]);
          ...
          <>
            <TableContainer component={Paper} sx={{ borderRadius: 3, boxShadow: 3 }}>
              <Table>
                ...
                <TableBody>
                  {data.map((item) => (
                    <ReturnRow key={item.returnOrderId} row={item} />
                  ))}
                </TableBody>
              </Table>
            </TableContainer>
            ...
            <Stack spacing={2} mt={3} alignItems="center">
              <Pagination
                ...
                onChange={(e, newPage) => setFilters((prev) => ({ ...prev, pageNumber: newPage }))}
                shape="rounded"
                showFirstButton
                showLastButton
              />
